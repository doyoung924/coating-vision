...
|FR-31|EWMA 평활 계수 λ 는 metric 별로 지정. 재귀식 `λ·value + (1-λ)·prev\_ewma`, 초기값 CENTER. 현재 값: **a3=0.1 · seg\_crack=0.2**. 근거: §23-3 격자 재해석 + §23-4a·§23-4b LOO (세그 8/8 non-negative, A3 8/8) 로 L=3.5 조건 각 metric F1 최적 λ 확인. 새 metric 추가 시 해당 metric 의 λ 값을 함께 지정해야 하며, 지정되지 않은 metric 은 SPC 처리에서 명확히 실패 (조용한 기본값 사용 금지). VALID\_METRICS 와 λ 매핑 키는 항상 일치해야 함. 상세 근거·재검증 조건은 `experiment\_log.md` §23-5|P0|`services/spc.py:EWMA\_LAMBDAS`, `\_next\_ewma`|`test\_spc.py::TestCalculationMatches09::test\_ewma\_formula`|
