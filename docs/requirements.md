...
|FR-31|EWMA λ=0.1. 재귀식 `λ·value + (1-λ)·prev\_ewma`. 초기값 CENTER. 원 채택 근거 (§9-3 lift) 폐기 후 §23-3 재해석에서 이 데이터 F1 상 λ=0.2 가 +0.20 우세 (LOO 8/8 non-negative §23-4a). **λ=0.1 유지 근거**: (1) 물리 근거 — 건조 크랙은 점진적 드리프트, λ 작을수록 추세 포착 유리 (2) 데이터셋 편의 (모두 CoatingVision) · 격자 선택 편의로 λ=0.2 재현성 미확인. **정량 근거 없음 → "물리 근거 있음, 이 데이터에서는 차선"으로 상태 정정**|P0|`services/spc.py:EWMA\_LAMBDA=0.1`, `\_next\_ewma`|`test\_spc.py::TestCalculationMatches09::test\_ewma\_formula`|
