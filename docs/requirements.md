...
|FR-30|관리한계: UCL = CENTER + 3.5σ (사용자 지시. 09\_spc\_monitor.py 원본 3.0σ 대체)|P0|`services/spc.py:SIGMA\_LIMIT=3.5`, `compute\_control\_limits`|`test\_spc.py::TestCalculationMatches09::test\_control\_limits\_formula`|
|FR-31|EWMA λ=0.1. 재귀식 `λ·value + (1-λ)·prev\_ewma`. 초기값 CENTER|P0|`services/spc.py:EWMA\_LAMBDA=0.1`, `\_next\_ewma`|`test\_spc.py::TestCalculationMatches09::test\_ewma\_formula`|
